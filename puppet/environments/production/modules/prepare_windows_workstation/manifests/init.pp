...
exec { 'enable_microsoft_update':
  ...
  unless    => 'powershell.exe -Command "(Get-WindowsUpdateLog)"',
  provider  => powershell,
}

# Install Windows Updates and accept the EULA
exec { 'install_windows_updates':
  command   => 'powershell.exe -Command "Install-WindowsUpdate -AcceptEula"',
  unless    => 'powershell.exe -Command "(Get-WindowsUpdate -IsPending)"',
  provider  => powershell,
}
